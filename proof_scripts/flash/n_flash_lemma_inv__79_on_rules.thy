(*  Title:      HOL/Auth/n_flash_lemma_inv__79_on_rules.thy
...
theory n_flash_lemma_inv__79_on_rules imports n_flash_lemma_on_inv__79
begin
section{*All lemmas on causal relation between inv__79*}
lemma lemma_inv__79_on_rules:
  assumes b1: "r \<in> rules N" and b2: "(f=inv__79  )"
  ...
  apply (cut_tac b1, auto) done
    moreover {
      ...
      apply (cut_tac b2 d1, metis n_StoreVsinv__79) done
    }

    moreover {
      assume d1: "(\<exists> data. data\<le>N\<and>r=n_Store_Home  data)"
      have "invHoldForRule s f r (invariants N)"
      apply (cut_tac b2 d1, metis n_Store_HomeVsinv__79) done
    }
