(*  Title:      HOL/Auth/n_flash_lemma_on_inv__10.thy
...
theory n_flash_lemma_on_inv__10 imports n_flash_base
begin
...
from a1 obtain src data where a1:"src\<le>N\<and>data\<le>N\<and>r=n_Store  src data" apply fastforce done
  have "?P3 s"
  apply (cut_tac a1 a2 , simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Para (Field (Ident ''Sta'') ''Proc'') src) ''CacheState'')) (Const CACHE_E)) (eqn (IVar (Field (Field (Ident ''Sta'') ''HomeProc'') ''CacheState'')) (Const CACHE_E))))" in exI, auto) done
