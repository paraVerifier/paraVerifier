(*  Title:      HOL/Auth/n_flash_lemma_on_inv__22.thy
...
theory n_flash_lemma_on_inv__22 imports n_flash_base
begin
...
from a1 obtain src data where a1:"src\<le>N\<and>data\<le>N\<and>r=n_Store  src data" apply fastforce done
  have "?P3 s"
  apply (cut_tac a1 a2 , simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Para (Field (Ident ''Sta'') ''Proc'') src) ''CacheState'')) (Const CACHE_E)) (eqn (IVar (Field (Field (Ident ''Sta'') ''HomeUniMsg'') ''Cmd'')) (Const UNI_PutX))))" in exI, auto) done
  then show "invHoldForRule s f r (invariants N)" by auto
qed

lemma n_Store_HomeVsinv__22:
...
from a1 obtain data where a1:"data\<le>N\<and>r=n_Store_Home  data" apply fastforce done
  have "?P3 s"
  apply (cut_tac a1 a2 , simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Field (Ident ''Sta'') ''HomeUniMsg'') ''Cmd'')) (Const UNI_PutX)) (eqn (IVar (Field (Field (Ident ''Sta'') ''HomeProc'') ''CacheState'')) (Const CACHE_E))))" in exI, auto) done
  then show "invHoldForRule s f r (invariants N)" by auto
qed
